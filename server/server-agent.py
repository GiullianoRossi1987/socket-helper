...
class ServerData(object):
    ...
    # connection data
    last_connections_today = list
    connected = bool()
    connected_with_ip = str 
    connected_with_port = int
    connected_type_connection = int
    ...
    def __init__(self, addr: tuple, tp_ip="host", action="--accept", tp_connection: str = "--message-tiping"):
        ...
        if action == "--accept":  # if the system will accept the connection or refuse it
            ...
            self.agent.sendto(b"You were refused by '"+self.server_data["ServerName"]+b"'\n Have a nice day!", address=addr)
    ...
    def get_database_data(self, ports_lists: list, user_data: list,
    ...
    def export_regist_data(self) -> str:
    ...
    def send_connection_message_to(self, addr: tuple):
        self.agent.connect(addr)
        self.agent.sendto(b"You're now connected with '"+bytes(self.server_data["Name"])+b"' !", address=addr)
        self.agent.close()
    
    def get_data_from(self, addr:tuple, tp_addr: Optional[str] = bytes):
        self.agent.connect(addr)
        n = self.agent.recv(1024)
...
class FtpAgentSystem(object):
    ...
    addr_data = list # probably exported from ServerData
    lasts_downloads = [
    ...
    def __init__(self, addr: list, timeout_connection: Optional[int] = 10):
